...
defmodule ExAdmin.CSVTest do
  ...
  test "generates default schema", %{data: data} do
    expected = ~w(id name description inserted_at updated_at)a
    |> Enum.map(&(%{field: &1, fun: nil}))
    assert CSV.default_schema(data) == expected
  end

  test "generates default csv", %{data: data}  do
    assert CSV.build_csv(data) == "Id,Name,Description,Inserted At,Updated At\n1,A,Azz,,\n2,B,Bzz,,\n3,C,Czz,,"
  end
  ...
  test "custom schema with fun don' humanize", %{data: data} do
    ...
    assert CSV.build_csv(schema, data, [humanize: false]) == "name,description\nMr. A,Azz\nMr. B,Bzz\nMr. C,Czz"
  end
  ...
  test "generates CSV with column", %{data: data} do
    assert TestExAdmin.CSV.Columns.build_csv(data) == "Name,Description\nMrs. A,Azz\nMrs. B,Bzz\nMrs. C,Czz"
  end
